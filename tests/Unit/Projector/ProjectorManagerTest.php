...
use Storm\Projector\ProjectorMonitor;
...
beforeEach(function () {
    ...
    $this->projectorManager = new ProjectorManager($this->subscriptionFactory);
});

test('default instance', function () {
    expect($this->projectorManager)->toBeInstanceOf(ProjectorManager::class)
        ->and($this->projectorManager)->toBeInstanceOf(ProjectorManagerInterface::class);
});
...
test('new query projector', function (array $options) {
    ...
    $projector = $this->projectorManager->newQueryProjector($options);
...
test('new emitter projector', function (string $streamName, array $options) {
    ...
    $projector = $this->projectorManager->newEmitterProjector($streamName, $options);
...
test('new read model projector', function (string $streamName, ReadModel $readModel, array $options) {
    ...
    $projector = $this->projectorManager->newReadModelProjector($streamName, $readModel, $options);
