...
test('skip activity when gap detected', function () {
    $this->hub->expects('expect')->with(HasGap::class)->andReturn(true);
    $this->hub->expects('notifyWhen')->never();

    ($this->activity)($this->hub, fn () => true);
});

test('sleep and update lock when no stream event to process', function () {
    $this->hub->expects('expect')->with(HasGap::class)->andReturn(false);
    $this->hub->expects('expect')->with(IsProcessBlank::class)->andReturn(true);
    $this->hub->expects('notifyWhen')->withArgs(
    ...
    $this->hub->expects('notify')->with(BatchSleep::class);
...
test('store stream event', function () {
    $this->hub->expects('expect')->with(HasGap::class)->andReturn(false);
    $this->hub->expects('expect')->with(IsProcessBlank::class)->andReturn(false);
    ...
    $this->hub->expects('notify')->with(BatchSleep::class)->never();
