...
test('sleep and update lock when no stream event to process', function () {
    ...
    $this->hub->shouldReceive('expect')
        ->with(IsProcessBlank::class)
        ->once()
        ->andReturn(true);

    $this->hub->shouldReceive('notifyWhen')
        ->once()
        ->withArgs(function (bool $condition, Closure $onSuccess) {
            $onSuccess($this->hub);

            return $condition === true;
        });

    $this->hub->shouldReceive('notify')
        ->once()
        ->with(BatchSleep::class);

    $this->hub->shouldReceive('trigger')
        ->once()
        ->withArgs(fn (object $trigger) => $trigger instanceof ProjectionLockUpdated);
...
test('store stream event', function () {
    ...
    $this->hub->shouldReceive('notifyWhen')
        ->once()
        ->withArgs(function (bool $condition, Closure $onSuccess, Closure $onFailure) {
            $onFailure($this->hub);
