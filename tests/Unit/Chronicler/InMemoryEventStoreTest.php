...
beforeEach(function () {
    ...
    $this->eventStore = new InMemoryEventStore($this->eventStreamProvider, new StreamCategoryDetector());
    $this->aggregateId = GenericAggregateId::fromString(Uuid::v4()->jsonSerialize());
});

dataset('stream names', [
    'stream' => ['stream1'],
    'stream with category' => ['category-foo'],
    'stream with category and many dash' => ['category-foo-bar'],
]);
...
test('filter stream', function (array $filter, array $expected) {
    ...
    expect($this->eventStore->filterStreams(...$filter))->toBe($expected)
        ->and($this->eventStore->filterCategories(...$filter))->toBeEmpty();
})
    ->with([
        ['filter' => [], 'expected' => []],
...
test('filter categories by splitting stream name with first dash', function () {
    $categories = ['stream-foo', 'stream-bar', 'stream-foobar', 'another_stream-foo'];
    ...
    expect($this->eventStore->filterCategories('category-1'))->toBeEmpty()
        ->and($this->eventStore->filterCategories('stream'))->toBe(['stream-foo', 'stream-bar', 'stream-foobar'])
        ->and($this->eventStore->filterCategories('another_stream'))->toBe(['another_stream-foo'])
        ->and($this->eventStore->filterCategories('stream-foo', 'stream-bar'))->toBeEmpty()
        ->and($this->eventStore->filterCategories('stream-foo', 'stream-bar', 'stream-foobar'))->toBeEmpty()
