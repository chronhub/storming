...
trait ReadModelDatabaseStack
{
    ...
    public function increment(string $id, string $column, int $amount): void
    {
        $this->stack[] = fn () => $this->query()->where($this->getKey(), $id)->increment($column, $amount);
    }

    public function incrementEach(string $id, array $columns, array $extra = []): void
    {
        $this->stack[] = fn () => $this->query()->where($this->getKey(), $id)->incrementEach($columns, $extra);
    }

    public function decrement(string $id, string $column, int $amount): void
    {
        $this->stack[] = fn () => $this->query()->where($this->getKey(), $id)->decrement($column, $amount);
    }

    public function decrementEach(string $id, array $columns, array $extra = []): void
    {
        $this->stack[] = fn () => $this->query()->where($this->getKey(), $id)->decrementEach($columns, $extra);
    }
